...
class DenseLayer(object):
    ...
    def createout(self, noise_lvl): #create output with a scalar noise parameter
        ...
        elif self.noise == 'special':
            print >> sys.stderr, 'special noise'
            self.lin = T.dot(self.inp, self.W) + self.b
            self.lin.name = 'lin'+self.tag
            self.out = self.mask * self.act_fn(self.noise_fn(self.theano_rng,self.lin,self.noise_lvl))
            self.out.name = 'out'+self.tag
        else:
    ...
    def __redefinemodel(self,direction,depth_max,depth_min=0,noise_lvl=0.,update_type = 'global'):
        ...
        if direction != 0:
            ...
            for i in itera:
                ...
                if direction == 1 and i==depth_min and self.noise[i]!='special': 
                    print >> sys.stderr, 'depth',i,self.noise[i]
                    tmp_layers[i].createout(noise_lvl)
                if len(self.noise)>i and direction == 1 and i==depth_min-1  and self.noise[i+1]=='special':
                    print >> sys.stderr, 'at layer',i,'change noise to special'
                    savenoise = tmp_layers[i].noise
                    tmp_layers[i].noise = 'special'
                    tmp_layers[i].noise_fn = eval(tmp_layers[i].noise+"_noise") if tmp_layers[i].noise != None else None
                    tmp_layers[i].createout(noise_lvl)
                    tmp_layers[i].noise = savenoise
                    tmp_layers[i].noise_fn = eval(tmp_layers[i].noise+"_noise") if tmp_layers[i].noise != None else None
