...
def LeaveOneOut(C,DataPath,depth,listtotest):
    if depth!=0:
        TrainVectors = DataPath + '_DL%s.vec'%(depth)
    else:
        TrainVectors = DataPath + '.vec'
...
if __name__ == '__main__':
    if len(sys.argv) <= 2:
        print "Usage:", sys.argv[0], "DataPath depth"
        sys.exit(-1)
    
    DataPath = sys.argv[1]
    depth = sys.argv[2]
    C,stdC = EstimateC(DataPath,depth)
    print "Chosen C:",C,"with std:",stdC
    listtotest = list(numpy.arange(8000))
    res,stdres = LeaveOneOutCheap(C,DataPath,depth,listtotest)
    print "Test accuracy",res,"with std:",stdres
